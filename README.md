...
- Materials move with **Ctrl + Alt + ← / →** anywhere on the computer, so the class screen advances while a browser or a
  PDF viewer has focus.
- Fewer buttons: the drawing toolbar lost seven of them, and every dialog now matches the console instead of using
...
- 판서 도형: 숫자키 **1** 자유, **2** 네모, **3** 동그라미, **4** 별. **마우스 휠**로 굵기 조절. 되돌리기·지우개·전체 지우기는 그대로.
- 자료 이동은 컴퓨터 어디서나 **Ctrl + Alt + ← / →**. 브라우저나 PDF 뷰어에 포커스가 있어도 넘어갑니다.
- 버튼 정리: 판서 도구 버튼 7개를 없애고 단축키로 옮겼으며, 모든 대화 창을 콘솔과 같은 디자인으로 통일했습니다.
